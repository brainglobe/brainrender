...
# Create a scene
scene = Scene(screenshot_kwargs=screenshot_params) # use screenshot_params to specify where the screenshots will be saved

scene.add_brain_regions(['TH'])

# render
scene.render(camera='top', interactive=False) # if interactive is false the program won't stop when the scene is rendered
