...
scene = Scene(add_root=False, 
        ...
        display_inset=False, 
        atlas_kwargs=dict(data_folder=data_folder), # use this to pass keyword arguments to the Atlas class
...
scene.add_neurons(list(neurons.neuron.values[:5]), as_skeleton=True) # You can render the neruons as just their skeleton

scene.add_neurons_synapses(list(neurons.neuron.values[:5]), pre=True, post=True)
"""
