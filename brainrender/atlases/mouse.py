...
from brainrender.Utils.ABA.aba_utils import parse_neurons_colors, parse_tractography_colors
from brainrender.Utils.data_manipulation import return_list_smart, is_any_item_in_list
...
class ABA(BrainGlobeAtlas):
    ...
    def get_tractography(self, tractography, color=None,  color_by="manual", others_alpha=1, verbose=True,
        ...
        COLORS = parse_tractography_colors(tractography, color=color, color_by=color_by, VIP_regions=VIP_regions, 
                                                        VIP_color=VIP_color, others_color=others_color)
        COLORS = [c if c is not None else self.get_region_color_from_acronym(t['structure-abbrev']) for c, t in zip(COLORS, tractography)]
        ...
        for i, (t, color) in enumerate(zip(tractography, COLORS)):
            ...
            if brainrender.VERBOSE and verbose and not is_any_item_in_list(inj_structures, structures_acronyms):
            ...
            if extract_region_from_inj_coords:
                try:
                    region = self.get_region_name_from_coords(t['injection-coordinates'])
                    if region is None: continue
                    inj_structures = [self.get_structure_parent(region['acronym'])['acronym']]
                except:
                    raise ValueError(self.get_region_name_from_coords(t['injection-coordinates']))
            ...
            if display_injection_volume:
                actors.append(shapes.Sphere(pos=t['injection-coordinates'],
                                c=color, r=brainrender.INJECTION_VOLUME_SIZE*t['injection-volume'], alpha=brainrender.TRACTO_ALPHA))

            points = [p['coord'] for p in t['path']]
            actors.append(shapes.Tube(points, r=brainrender.TRACTO_RADIUS, c=color, alpha=alpha, res=brainrender.TRACTO_RES))
